...
<body>
    <div style="background: #A7A7A7;">
        ...
        &emsp;<button class="button" onclick="window.location.href='startedpickup.php'"><b>Back</b></button>
        </center><br>
    </div>
    <br>
    <main>
    ...
    $id3 = $_GET['id'];
    if (!empty($sku)) {
    $sql1 = "SELECT * FROM pickupdetails WHERE skucode = '$sku' AND  pickupid = '$id3'";
    ...
    $id3 = $_GET['id'];
    $sql3 = "SELECT * FROM pickupdetails WHERE pickupid = '$id3'";
    ...
    <div class="container" style="background: white">
        ...
        <header><center>Check Item</center></header>
            <form action="" method="post">
                ...
                <div class="field input">
                    <label for="pqty"><b><pre>Picked QTY</pre></b></label>
                    <input type="text" name="pqty" id="pqty" autocomplete="off" readonly>
                <center>
                ...
                <div class="field input">
                    <label for="caseno"><b><pre>Case No</pre></b></label>
                    <input type="text" name="caseno" id="caseno" autocomplete="off" readonly>
                </div>
    ...
    function confirm(id,id3) {
        ...
        xhr.open("POST", "confirmp.php", true);
    ...
    function plus(id2,id4) {
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "plusp.php", true);
        xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                var responseText = xhr.responseText;
                var jsonResponse = JSON.parse(responseText);
                var pqty2 = jsonResponse.pqty2;
                document.getElementById("pqty").value = pqty2;
            }
        };
    ...
    if(isset($_POST['end'])){
        ...
        $id3 = $_GET['id'];
        $usql = "UPDATE `pickup` SET `confirm`='finished',`endtime`='$current_time' WHERE id = '$id3'";
        $uresult = $con->query($usql);
        echo "<script>window.location.href='pickup1.php'</script>";
    }
    ?>
    ...
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "checkitemp.php", true);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4 && xhr.status == 200) {
                var responseText = xhr.responseText;
                ...
                var pqty2 = jsonResponse.pqty2;
                var caseno2 = jsonResponse.caseno2;
                ...
                document.getElementById("pqty").value = pqty2;
                document.getElementById("caseno").value = caseno2;
